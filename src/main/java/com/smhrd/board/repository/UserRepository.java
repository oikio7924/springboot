...
@Repository // 어노테이션 안써도 됨 -> extends JpaRepository 를 해주었기 때문
public interface UserRepository extends JpaRepository<UserEntity, Long>{
	// 1. 인터페이스 상속 받기
	// 인터페이스간 상속은 extends 키워드 사용
	
	// <> 제너릭 : 레퍼런스 타입만 가능 --> 기본타입 8가지는 들어갈 수 없음
	...
	// ID: entity 가 가지고 있는 pk 필드의 타입(@Id가 걸려있는 필드의 타입)
	
	// 메소드의 기능 (기본적으로 내장 되어 있는 메소드 소개)
	
	/* 1. save(entity)
	 * 	--> insert into entity 명(table) values(매개변수로 넣은 entity);
	 
	 * 2. delete(매개변수)
	 * 	--> delete from entity 명 where pk 명 = 매개변수;
	 
	 * 3. findAll()
	 * 	--> select * from entity 명;
	 
	 * 4. findById(매개변수)
	 * 	--> select * from entity 명 where id = 매개변수;
	...
	// null 이 왔을 시 프로그램 종료의 위험을 막기 위해 -> 값이 없을 시 empty 를 return
