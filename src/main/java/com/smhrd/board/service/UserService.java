...
public class UserService {
	...
	/*
	 * Spring 에서는 3계층 구조를 띔
	 * 
	 * [Spring 3계층]
	 * Controller - Service - Repository(mapper)
	 * 
	 * - Controller : mapping 처리, HTTP 요청 처리, 사용자와 직접적인 상호 작용
	 * - Service : 비지니스 로직 담당, Repository(mapper) 연결
	 * - Repository(mapper) : 데이터베이스와 직접 통신
	 * 
	 * [Controller 가 Repository 와 직접적인 연결을 하지 않는 이유]
	 * 1. Controller 와 Repository 가 직접적인 연결을 한다면
	 * 		Controller 의 역할이 많아지며 가독성이 낮아지고, Repository 결합도가 올라감
	 * 2. 유지보수가 힘들어짐
	 * 3. 여러개의 쿼리문 사용이 힘들어짐
	 * 
	 * [Service 객체를 사용하는 이유]
	 * - 설계에 따라 유지보수성을 올릴 수 있음 (응높 결낮)
	 * 
	 * 결론 : Controller 는 Service 와 연결 / Service 객체는 Repository 와 연결
	 * 		 
	 * */
